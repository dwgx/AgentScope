const { contextBridge, ipcRenderer } = require("electron");

contextBridge.exposeInMainWorld("agentscope", {
  getSnapshot: () => ipcRenderer.invoke("snapshot:get"),
  getDoctor: () => ipcRenderer.invoke("doctor:get"),
  search: (query, limit = 50, options) => ipcRenderer.invoke("search:run", query, limit, options),
  exportSnapshot: () => ipcRenderer.invoke("snapshot:export"),
  getAppInfo: () => ipcRenderer.invoke("app:info"),
  setControlMode: (mode) => ipcRenderer.invoke("app:setControlMode", mode),
  listFonts: () => ipcRenderer.invoke("fonts:list"),
  listCodexControl: () => ipcRenderer.invoke("codexControl:list"),
  getCodexControlCenter: () => ipcRenderer.invoke("codexControl:center"),
  readCodexControlDocument: (id) => ipcRenderer.invoke("codexControl:read", id),
  revealCodexControlSurface: (id) => ipcRenderer.invoke("codexControl:revealSurface", id),
  saveCodexControlDocument: (id, content, expectedSha256) =>
    ipcRenderer.invoke("codexControl:save", id, content, expectedSha256),
  readCodexModeConfig: () => ipcRenderer.invoke("codexControl:readModes"),
  saveCodexModeConfig: (patch, expectedSha256) =>
    ipcRenderer.invoke("codexControl:saveModes", patch, expectedSha256),
  listCodexConfigTemplates: () => ipcRenderer.invoke("codexControl:listTemplates"),
  saveCodexConfigTemplate: (template) => ipcRenderer.invoke("codexControl:saveTemplate", template),
  deleteCodexConfigTemplate: (id) => ipcRenderer.invoke("codexControl:deleteTemplate", id),
  previewCodexConfigTemplate: (request) => ipcRenderer.invoke("codexControl:previewTemplate", request),
  getCodexConfigWorkbench: () => ipcRenderer.invoke("codexControl:workbench"),
  planCodexControlMutation: (request) => ipcRenderer.invoke("codexControl:planMutation", request),
  executeCodexControlMutation: (request) => ipcRenderer.invoke("codexControl:executeMutation", request),
  pickCodexConfigPath: (kind) => ipcRenderer.invoke("codexControl:pickPath", kind),
  reloadApp: () => ipcRenderer.invoke("app:reload"),
  clearCache: () => ipcRenderer.invoke("app:clearCache"),
  quitApp: () => ipcRenderer.invoke("app:quit"),
  openExternal: (url) => ipcRenderer.invoke("shell:openExternal", url),
  openPath: (targetPath) => ipcRenderer.invoke("shell:openPath", targetPath),
  revealPath: (targetPath) => ipcRenderer.invoke("shell:revealPath", targetPath),
  inspectPid: (pid) => ipcRenderer.invoke("inspect:pid", pid),
  inspectSession: (sessionId) => ipcRenderer.invoke("inspect:session", sessionId),
  repairDiagnostic: (name) => ipcRenderer.invoke("diagnostic:repair", name),
  backupSession: (agent, sessionId) => ipcRenderer.invoke("session:backup", agent, sessionId),
  deleteSession: (agent, sessionId, createdAt, options) => ipcRenderer.invoke("session:delete", agent, sessionId, createdAt, options),
  launchSession: (agent, sessionId, action, context) => ipcRenderer.invoke("session:launch", agent, sessionId, action, context),
  importSessionBackup: (backupDir) => ipcRenderer.invoke("session:import", backupDir),
  listQuarantinedSessions: () => ipcRenderer.invoke("session:listQuarantine"),
  restoreQuarantinedSession: (quarantineDirOrJournalPath) => ipcRenderer.invoke("session:restore", quarantineDirOrJournalPath),
  chooseImportSession: () => ipcRenderer.invoke("session:chooseImport"),
  writeDeletePlan: (agent, sessionId) => ipcRenderer.invoke("session:deletePlan", agent, sessionId),
  writeImportPlan: (backupDir) => ipcRenderer.invoke("session:importPlan", backupDir),
  chooseImportPlan: () => ipcRenderer.invoke("session:chooseImportPlan")
});
