...
async function smokeCodexControlInteractions(page) {
  ...
  await page.locator('[data-testid="codex-control-tabs"] button[data-value="templates"]').click();
  await page.locator('[data-testid="codex-config-workbench"]').waitFor();
  await page.locator('[data-testid="codex-workbench-section-current"]').waitFor();
  await page.locator('[data-testid="codex-workbench-section-mcp"]').waitFor();
  await page.locator('[data-testid="codex-workbench-section-unknown"]').waitFor();
  await page.locator('[data-testid="codex-workbench-section-mcp"]').click();
  await page.locator('[data-testid="codex-mcp-server-group"][data-server-name="playwright"]').waitFor();
  await page.locator('[data-testid="codex-workbench-item"][data-item-id="config.mcp_servers.playwright.command"]').waitFor();
  await page.locator('[data-testid="codex-workbench-item"][data-item-id="config.mcp_servers.playwright.args"]').waitFor();
  const mcpToggle = page.locator('[data-testid="codex-mcp-server-enabled-playwright"]');
  await mcpToggle.waitFor();
  await mcpToggle.click();
  await page.locator('[data-testid="codex-mcp-server-group"][data-server-name="playwright"].disabled').waitFor();
  const mcpSectionCount = await page.locator('[data-testid="codex-workbench-section-mcp"] span').innerText();
  if (mcpSectionCount.trim() !== "1") throw new Error(`Expected one MCP server group, found ${mcpSectionCount}`);
  await expectText(page.locator('[data-testid="codex-workbench-stage"]'), /mcp_servers\.playwright\.enabled/i);
  await assertNoHorizontalOverflow(page, "Codex Control MCP workbench");
  await page.screenshot({ path: path.join(outputRoot, "codex-control-mcp-workbench.png"), fullPage: true });
  await page.locator('[data-testid="codex-workbench-apply"]').click();
  await page.locator('[data-testid="codex-apply-modal"][data-phase="success"]').waitFor({ timeout: 25_000 });
  await waitForConfigContains("enabled = false");
  await waitForCodexControlJournal("mcp_servers.playwright.enabled");
  await page.locator('[data-testid="codex-apply-close"]').click();
  await mcpToggle.click();
  await page.locator('[data-testid="codex-mcp-server-group"][data-server-name="playwright"].enabled').waitFor();
  await page.locator('[data-testid="codex-workbench-apply"]').click();
  await page.locator('[data-testid="codex-apply-modal"][data-phase="success"]').waitFor({ timeout: 25_000 });
  await waitForConfigContains("enabled = true");
  await page.locator('[data-testid="codex-apply-close"]').click();
  await page.locator('[data-testid="codex-workbench-section-templates"]').click();
  await page.locator('[data-testid="codex-template-card"][data-template-id="builtin.yolo-full-access"]').click();
  await page.locator('[data-testid="codex-template-row"][data-item-id="config.approval_policy"]').waitFor();
  await page.locator('[data-testid="codex-template-row"][data-item-id="config.web_search"] input').uncheck();
  await page.getByRole("button", { name: /加入待应用|Stage template/i }).click();
  await assertNoHorizontalOverflow(page, "Codex Control templates");
  await page.screenshot({ path: path.join(outputRoot, "codex-control-templates.png"), fullPage: true });
  await page.locator('[data-testid="codex-workbench-apply"]').click();
  await page.locator('[data-testid="codex-apply-modal"]').waitFor();
  await page.screenshot({ path: path.join(outputRoot, "codex-control-template-apply-modal.png"), fullPage: true });
  await page.locator('[data-testid="codex-apply-modal"][data-phase="success"]').waitFor({ timeout: 25_000 });
  await page.locator('[data-testid="codex-apply-close"]').click();
  await waitForConfigMissing('web_search = "live"');
  await waitForConfigContains('approval_policy = "never"');
  ...
  await page.locator('[data-testid="codex-control-item-config.sandbox_mode-option"][data-value="workspace-write"]').click();
  await waitForEnabled(save, "Codex Control safety save");
  await save.click();
  await waitForNotification(page, /Codex|淇濆瓨|saved|control/i);
  await waitForConfigContains('sandbox_mode = "workspace-write"');
  await page.locator('[data-testid="notification-close"]').click();
  await page.locator('[data-testid="codex-control-item-config.sandbox_mode-trigger"]').click();
